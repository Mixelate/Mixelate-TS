import { Events, EmbedBuilder, ModalBuilder, TextInputBuilder, TextInputStyle, ActionRowBuilder, StringSelectMenuBuilder, ButtonBuilder, ButtonStyle, Role } from "discord.js";
...
const { startOrder, startApplication, requestSupport, printRoles } = require("./utils/helper");
...
client.on(Events.InteractionCreate, async (interaction: any) => {
	...
	if (interaction.isChatInputCommand()) {
		...
		if (splitArray[0] === "help") {
			...
			switch (splitArray[1]) {
				...
				case "close": {
					interaction.
					interaction.reply({
						embeds: [new EmbedBuilder().setColor(client.config.embedColor).setAuthor({ name: `This ticket has been scheduled to close in ${ms(ms(`30m`))}.`, iconURL: `${interaction.guild?.iconURL()}` })]
					}).then(async (msg: any) => {
						setTimeout(function () {
		...
		if (selected[0] === "support") {
			...
			const support = interaction.guild.roles.cache.find((r: any) => r.id === client.config.roles.support);
			switch (selected[1]) {
				case "transaction": {
					embed.addFields({
						name: "Details",
